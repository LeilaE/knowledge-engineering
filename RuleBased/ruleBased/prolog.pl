...
person(milos).
age(milos,23).
activity(milos,inactive).
smoker(milos,yes).
pregnant(milos,no).
genetics(milos,hypotension).
genetics(milos,leukemia).
creatinine_level(mihajlo,normal).
albumin_level(mihajlo,high).
calcium_level(mihajlo,high).
ldh_level(mihajlo,high).
pcr_chromosome_changes(nikola,low).
lymphoma_cells_level(nikola,normal).
iron_check(nikola,low).
hemoglobin_check(nikola,normal).
b12_check(nikola,low).
folic_acid_check(nikola,high).
person(miodrag).
age(miodrag,52).
activity(miodrag,inactive).
smoker(miodrag,yes).
pregnant(miodrag,no).
genetics(miodrag,thrombocytosis).
genetics(miodrag,hemophilia).
